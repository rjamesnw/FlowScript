...
declare namespace AceAjax {
    ...
    export interface CommandManager extends MultiHashHandler, EventEmitter {
    ...
    export interface KeyBindingHandler {
        (data?: any, hashId?: number, keyString?: string, keyCode?: number, e?: KeyboardEvent): any
    }

    export interface KeyBinding {

        setDefaultHandler(kb: KeyBindingHandler | { handleKeyboard: KeyBindingHandler }): void;

        setKeyboardHandler(kb: KeyBindingHandler | { handleKeyboard: KeyBindingHandler }): void;

        addKeyboardHandler(kb: KeyBindingHandler | { handleKeyboard: KeyBindingHandler }, pos?: number): void;

        removeKeyboardHandler(kb: KeyBindingHandler | { handleKeyboard: KeyBindingHandler }): boolean;
    ...
    **/
    export interface IEditSession extends EventEmitter {
        ...
        //on(event: string, fn: (e: any) => any): void;
        on<T extends IEventEmitterCallback<any>>(eventName: string, callback: T, capturing?: boolean): T;
    ...
    **/
    export interface Editor extends EventEmitter {
        ...
        addEventListener(ev: string, callback: (ev: EditorChangeEvent) => any): void;
    ...
    **/
    ...
    export interface OOP {
        ...
        mixin(obj: {}, mixin: {}): {};
        implement(proto: {}, mixin: {}): {};
    }

    export interface Mirror {
        /** Note: The default timeout is initialized to 500ms. */
        setTimeout(timeout: number): void;
        setValue(value: any): void;
        getValue(callbackId: string): void;
        /** Note: Abstract method [does nothing by default] - should be overridden in derived types if required */
        onUpdate(): void;
        isPending(): boolean;
    }
    var Mirror: { new (sender: Sender): void };

    export interface IDeferredCall {
        (...args: any[]): any;
        schedule: IDeferredCall;
        call(): IDeferredCall;
        cancel(): IDeferredCall;
        isPending(): boolean;
    }

    export interface IDelayedCall {
        (timeout: number): void;
    ...
    export interface Lang {
        last(a: any[]): any;
        stringReverse(string: string): string;
        stringRepeat(string: string, count: number): string;
        stringTrimLeft(string: string): string
        stringTrimRight(string: string): string;
        copyObject<T extends {}>(obj: T): T;
        copyArray<T>(array: T[]): T[];
        deepCopy<T extends {}>(obj: T): T;
        arrayToMap(arr: string[]): { [index: string]: number };
        createMap(props: {}): { [index: string]: any };
        ...
        getMatchOffsets(string: string, regExp: RegExp): { offset: number; length: number; }[];

        /** deprecated */
        deferredCall(fcn: Function): IDeferredCall;

        delayedCall(fcn: Function, defaultTimeout: number): IDelayedCall;
    }

    export interface WorkerClient extends EventEmitter {
        onMessage(e: MessageEvent): void;
        reportError(err: string): void;
        $normalizePath(path: string): string;
        terminate(): void;
        send(cmd: string, args: any[]): void;
        call(cmd: string, args: any[], callback: Function): void;
        emit(event: string, data: WorkerEvent<any>): void;
        attachToDocument(doc: Document): void;
        changeListener(delta: Delta): void;
        $sendDeltaQueue(): void;
        $workerBlob(workerUrl: string): Blob;
    }
    var WorkerClient: {
        new (topLevelNamespaces: string[], mod: string, classname: string, workerUrl?: string): WorkerClient;
    }

    export interface UIWorkerClient extends WorkerClient {
        callbackId: number;
        ...
        setEmitSync(val: any): void;
    }
    var UIWorkerClient: {
        new (topLevelNamespaces: string[], mod: string, classname: string): UIWorkerClient;
    }

    interface ACEEvent { type?: string; stopPropagation(): void; preventDefault(): void; }
    interface IEventEmitterCallback<T extends ACEEvent> { (event: T, _this?: EventEmitter): void }

    export interface EventEmitter {
        _emit(eventName: string, e: ACEEvent): void;
        _dispatchEvent(eventName: string, e: ACEEvent): void;
        _signal(eventName: string, e: ACEEvent): void;

        once<T extends IEventEmitterCallback<any>>(eventName: string, callback: T): void;
        setDefaultHandler<T extends IEventEmitterCallback<any>>(eventName: string, callback: T): void;
        removeDefaultHandler<T extends IEventEmitterCallback<any>>(eventName: string, callback: T): void;

        on<T extends IEventEmitterCallback<any>>(eventName: string, callback: T, capturing?: boolean): T;
        addEventListener<T extends IEventEmitterCallback<any>>(eventName: string, callback: T, capturing?: boolean): T;

        off<T extends IEventEmitterCallback<any>>(eventName: string, callback: T): void;
        removeListener<T extends IEventEmitterCallback<any>>(eventName: string, callback: T): void;
        removeEventListener<T extends IEventEmitterCallback<any>>(eventName: string, callback: T): void;

        removeAllListeners(eventName: string): void;
    }

    export interface Sender extends EventEmitter {
        callback(data: any, callbackId: string): void;
        emit(name: string, data?: {}): void;
    }

    export interface WorkerEvent<T> extends ACEEvent {
        name: string,
        data: T
    }

    export interface IKey { key: string, hashId: number }

    export type IKeyBindingItem = string | KeyBindingHandler | EditorCommand;
    export interface IKeyBindingList { [name: string]: IKeyBindingItem }

    export interface HashHandler {
        platform: string;
        commands: { [name: string]: EditorCommand };
        /** Holds key bindings for single commands, or multiple commands. */
        commandKeyBinding: { [keyid: string]: string | EditorCommand | EditorCommand[] };

        addCommand(command: EditorCommand): void;
        removeCommand(command: string | EditorCommand, keepCommand: boolean): void;
        bindKey(key: any, command: IKeyBindingItem, position: number): EditorCommand | void;
        getPosition(command: EditorCommand): void;
        addCommands(commands: IKeyBindingList): void;
        removeCommands(commands: IKeyBindingList): void;
        bindKeys(keyList: IKeyBindingList): void;
        parseKeys(keys: string): IKey;
        findKeyCommand(hashId: number, keyString: string): string | EditorCommand | EditorCommand[];
        handleKeyboard(data: any, hashId: number, keyString: string, keyCode: number): { command: string | EditorCommand };
        getStatusText(editor: Editor, data: any): string;
    }

    export interface MultiHashHandler extends HashHandler { }

    var HashHandler: { new (config?: { [name: string]: (string | KeyBindingHandler | EditorCommand) }, /**"mac" or "win", empty = auto detect.*/platform?: string): HashHandler };
    var MultiHashHandler: { new (config?: { [name: string]: (string | KeyBindingHandler | EditorCommand) }, /**"mac" or "win", empty = auto detect.*/platform?: string): MultiHashHandler };
}
