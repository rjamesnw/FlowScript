namespace FlowScript.UI {
    ...
    export class ScriptEditor {
        ...
        monacoEditor: FlowScript.UI.Monaco.MonacoEditor;
        ...
        typePrompt(ok: { (node: ITreeViewItem<NamespaceObject>): boolean }, msg?: string) {
            ...
            this.typeTree._onTypeChanged = (node) => {
                if (node.$__treeItem instanceof Component) {
                    ...
                    if (comp.componentType == ComponentTypes.Code) {
                        this.monacoEditor.setCode((<FlowScript.Core.Code>comp).Code || "");
                    }
                }
         ...
         */
        private async _initUI_CodeOutput() {
            ...
            this.monacoEditor = new FlowScript.UI.Monaco.MonacoEditor(this.view, "tseditor", "tsoutput");

            await this.monacoEditor.initialize();

            //this.editor.loadLibFiles(["Scripts/FlowScript/ace/environments/serverlib.d.ts"]);
            this.monacoEditor.loadFile("Scripts/FlowScript/ace/environments/empty.ts");
            this.view.getElementById('btnSaveCode').onclick = () => {
                (<FlowScript.Core.Code>this.currentProject.currentComponent).Code = this.monacoEditor.getCode();
                alert("Saved.");
            };
        }
